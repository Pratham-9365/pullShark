...
const RepoDashboard = () => {
  ...
  const { repos, loading, error, pagination } = useSelector((state) => state.repos);
        ...
        <main className="relative z-10 w-full max-w-[1300px] h-full md:h-[85vh] md:mx-8 flex flex-col bg-zinc-900/30 backdrop-blur-xl md:rounded-3xl border border-white/10 shadow-2xl shadow-black/40 overflow-hidden ring-1 ring-white/5">
          ...
          <header className="flex-none px-8 py-5 border-b border-white/5 flex items-center justify-between bg-black/10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-semibold tracking-tight text-white">Repository Explorer</h1>
                <p className="text-xs text-zinc-500 font-medium tracking-wide uppercase">Project Index</p>
              </div>
            </div>
...
const RepoCard = ({ repo, index }) => (
  <motion.div
    ...
    transition={{ duration: 0.3, delay: index * 0.04, ease: [0.23, 1, 0.32, 1] }} // Modern ease curve
...
const ShimmerGrid = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
    {[...Array(9)].map((_, i) => (
      ...
      <div key={i} className="relative h-[200px] rounded-2xl bg-white/[0.02] border border-white/5 p-6 flex flex-col justify-between overflow-hidden">
        ...
        <div className="space-y-4 relative z-10">
           <div className="flex justify-between items-center">
             <div className="h-5 w-1/3 bg-zinc-800/50 rounded-md"></div> {/* Title placeholder */}
             <div className="h-5 w-12 bg-zinc-800/50 rounded-md"></div> {/* Language placeholder */}
           </div>
           <div className="space-y-2">
             <div className="h-4 w-full bg-zinc-800/30 rounded-md"></div> {/* Desc line 1 */}
             <div className="h-4 w-4/5 bg-zinc-800/30 rounded-md"></div> {/* Desc line 2 */}
           </div>
        </div>
        
        <div className="flex justify-between items-center pt-4 border-t border-white/5 relative z-10 mt-auto">
            <div className="flex gap-3">
                <div className="h-4 w-10 bg-zinc-800/50 rounded-md"></div> {/* Stats placeholder */}
                <div className="h-4 w-10 bg-zinc-800/50 rounded-md"></div> {/* Stats placeholder */}
            </div>
            <div className="h-4 w-16 bg-zinc-800/50 rounded-md"></div> {/* Button placeholder */}
        </div>
      </div>
    ))}
  </div>
